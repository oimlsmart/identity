...
| `POST /api/op/accounts` · `GET /api/op/accounts` · `POST /api/op/accounts/:id/enrollment` | The admin's invite surface (admin/cs_admin): create the account + its one-time link (emailed when the mailer is configured; the response's `mail` block says), list EVERY sign-in account with its sign-in posture (the OP's password accounts AND the seed-managed demo cast on a demo-postured deployment, each row marked by its `provider`; the erased tombstones stay out; never credentials), re-issue a link. The per-id acts serve the OP's own (`provider: password`) accounts only. |
