...
describe('the instrument\'s attribution — exact under concurrency (the 2026-09-18 wire lesson)', () => {
  ...
  it('the bootstrap never rides anonymous public reads (the 2026-09-18 production lesson: a failing seed retried in every /api/op/* window)', async () => {
    // The production shape: the account + client seeds DECLARED, and
    // the anonymous register read must carry ONLY its own 2 calls —
    // never the bootstrap's. The 2026-09-18 wire arithmetic: with the
    // seed failing (and evicting itself), every /api/op/* request
    // re-ran the declared rows' cost on top of its own — 22 calls
    // where 2 were the endpoint's. The fix scopes the bootstrap to
    // the credential surfaces that consume the declared rows.
    process.env.SERVER_TIMING = '1'
