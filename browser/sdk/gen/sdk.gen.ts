 ...
 *
 * The OIDC token endpoint. The machine grant is the RFC 8693 token exchange: `grant_type=urn:ietf:params:oauth:grant-type:token-exchange` with `subject_token_type=urn:oimlsmart:params:oauth:token-type:pat` and the personal access token as `subject_token` — answering a short-lived, scope-narrowed OP JWT. The token's scopes are re-judged against the holder's LIVE standing at every exchange. The RFC 8628 device_code leg (TODO.ai-platform/10) polls the device flow's ceremony: authorization_pending / slow_down / access_denied / expired_token while it runs, and on approval the ONE-TIME answer carrying the freshly minted personal access token's plaintext.
 */
 ...
 *
 * The CLI cone's user-attended bootstrap (TODO.ai-platform/10). The PUBLIC client (a secretless, application-class row — a confidential client has the code flow, a machine class has client_credentials) names its scope ask in the PAT grammar (`<service>:<read|write|admin>` space-joined, every service a registered, active, application-class relying party). The answer carries the device_code (the poll credential for /op/token's device_code leg), the user_code (what the holder types at the verification URI), both verification URIs, the expiry, and the poll interval. The approval (the browser leg at /op/device) re-judges the ask against the approving account's live standing; the successful poll mints the personal access token — the delivered credential IS the PAT, exchanged at the RFC 8693 grant per use exactly like a console-minted token.
 */
export const deviceAuthorization = <ThrowOnError extends boolean = false>(options: Options<DeviceAuthorizationData, ThrowOnError>): RequestResult<DeviceAuthorizationResponses, DeviceAuthorizationErrors, ThrowOnError> => (options.client ?? client).post<DeviceAuthorizationResponses, DeviceAuthorizationErrors, ThrowOnError>({
    ...urlSearchParamsBodySerializer,
 ...
 *
 * The crawl stays OPEN (User-agent: *, Allow: /) — the owner directive's noindex rides the meta tag on every page and the X-Robots-Tag header on every answer, so a Disallow here would trap the stale index entries: a crawler that cannot fetch never sees the noindex.
 */
